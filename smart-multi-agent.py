...
requirements: langchain-openai==0.1.25, langgraph
version: 0.8
licence: MIT
"""
...
class Pipe:
    class Valves(BaseModel):
        ...
        OPENAI_API_KEY: str = Field(default="ollama", description="Primary API key")
        MODEL_PREFIX: str = Field(default="SMART", description="Prefix before model ID")
        SMALL_MODEL: str = Field(
            default="qwen2.5:7b-instruct-fp16", description="Model for small tasks"
        )
        LARGE_MODEL: str = Field(
            default="qwen2.5:14b-instruct-q4_K_M", description="Model for large tasks"
        )
        ...
        ONLINE_MODEL: str = Field(
            default="openai/chatgpt-4o-latest", description="Online Model"
        )
        REASONING_MODEL: str = Field(default="anthropic/claude-3.5-sonnet")
        ...
        try:
            ...
            if is_reasoning_needed == "NO":
                messages_to_use[0]["content"] = (
                    messages_to_use[0]["content"] + USER_INTERACTION_PROMPT
                )
